 ...
 * Version 2.0 - Extended with full API coverage
 */

class MetabaseClient {
  constructor(metabaseUrl, apiKey) {
  ...
  async makeRequest(endpoint, options = {}) {
    ...
    try {
      ...
      console.error(`Metabase API Error for ${endpoint}:`, error);
      throw error;
    }
  }
   ...
   * @returns {Promise<Object>} Card information including SQL query
   */
   ...
   * @param {string} filter - Filter type ('all', 'mine', 'bookmarked', etc.)
   * @param {number} modelId - Optional model ID for filtering
   * @returns {Promise<Array>} Array of card summaries
   */
   ...
   * @param {number} cardId - The card ID to execute
   ...
   * @param {number} cardId - The card ID
   * @returns {Promise<string|null>} SQL query string or null
   */
   ...
   * @returns {Promise<Object>} Dashboard information with all cards
   */
   ...
   * @returns {Promise<Array>} Array of database summaries
   */
  async listDatabases() {
   ...
   * @param {number} databaseId - The database ID
   * @returns {Promise<Object>} Complete database metadata
   */
   ...
   * @returns {Promise<Object>} Table metadata with all fields
   */
  async getTableMetadata(tableId) {
    if (!tableId || tableId < 1) {
      throw new Error('Table ID must be a positive integer');
    }
   ...
   * @param {string} collectionId - The collection ID (use "root" for root)
   * @param {Array<string>} models - Optional filter by item type
   * @returns {Promise<Object>} Collection items
   */
   ...
   * @param {string} searchTerm - Term to search for
   * @returns {Promise<Array>} Matching dashboards
   */
   ...
   * @param {number} databaseId - The database ID
   * @returns {Promise<Array>} Cards for the specified database
   */
  ...
  async testConnection() {
    try {
      ...
      console.error('Metabase connection test failed:', error);
      return false;
    }
  }
   ...
   * @param {number} cardId - The card ID
   * @param {Object} parameters - Query builder parameters
   * @returns {Promise<Object>} Generated SQL information
   */
...
// Factory functions
export function createMetabaseClient(metabaseUrl, apiKey) {
  return new MetabaseClient(metabaseUrl, apiKey);
}
